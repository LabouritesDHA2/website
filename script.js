...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const offeringsOverlay = document.querySelector('.offerings-bg-overlay');
    const gradient = 'linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3))';

    function setOfferingsBg(bgImage, animate) {
        if (!offeringsSection || !bgImage) return;
        const fullBg = `${gradient}, url('${bgImage}')`;

        if (!animate || !offeringsOverlay) {
            offeringsSection.style.backgroundImage = fullBg;
            return;
        }

        // Sync base to what's currently displayed before starting a new crossfade
        // (handles rapid tab switching cleanly)
        if (offeringsOverlay._pendingBg) {
            offeringsSection.style.backgroundImage = offeringsOverlay._pendingBg;
        }
        offeringsOverlay._pendingBg = fullBg;

        // Instantly reset overlay, load new image, then fade in
        offeringsOverlay.style.transition = 'none';
        offeringsOverlay.style.opacity = '0';
        offeringsOverlay.style.backgroundImage = fullBg;

        requestAnimationFrame(() => requestAnimationFrame(() => {
            offeringsOverlay.style.transition = 'opacity 0.6s ease';
            offeringsOverlay.style.opacity = '1';
        }));
    }
    ...
    // Initialize background without animation on page load
    const activeTab = document.querySelector('.tab.active');
    if (activeTab) setOfferingsBg(activeTab.getAttribute('data-bg'), false);
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            setOfferingsBg(tab.getAttribute('data-bg'), true);
        });
    });
});
